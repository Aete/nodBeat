...
export const useNodDetection = (videoRef: React.RefObject<HTMLVideoElement>) => {
  ...
  useEffect(() => {
        ...
        if (results.faceLandmarks && results.faceLandmarks.length > 0) {
          ...
          if (lastYRef.current !== 0) {
            ...
            if (Math.abs(deltaY) > thresholdRef.current) {
              ...
              if (directionRef.current === 'up' && currentDirection === 'down') {
    ...
    const handleNod = (time: number) => {
      ...
      nodTimestampsRef.current = nodTimestampsRef.current.filter(t => time - t < 5000);

      if (nodTimestampsRef.current.length >= 2) {
        const first = nodTimestampsRef.current[0];
        const last = nodTimestampsRef.current[nodTimestampsRef.current.length - 1];
  ...
  return { landmarks, bpm, isNodding };
};
